...
ENSTRUMANLAR = {
    "ALTIN": {
        "futures":  "GC=F",
        "stooq":    "GC=F",     # yfinance günlük futures = spot referans (rollover var ama S1 için yeterli)
    ...
    "GUMUS": {
        "futures":  "SI=F",
        "stooq":    "SI=F",     # yfinance günlük futures = spot referans
...
def _stooq_gunluk(sembol: str, gun: int = 60) -> Optional[pd.DataFrame]:
    ...
    try:
        # Stooq sembolleri büyük harf duyarlı olabilir
        sembol = sembol.upper()
        url = f"https://stooq.com/q/d/l/?s={sembol}&i=d"
        r = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
        if r.status_code != 200:
            print(f"  ⚠️  Stooq {sembol}: HTTP {r.status_code}")
            return None
        if "Date" not in r.text:
            print(f"  ⚠️  Stooq {sembol}: CSV header 'Date' bulunamadı (bot koruması?)")
            return None
        df = pd.read_csv(StringIO(r.text))
        if df.empty or len(df) < 3:
            print(f"  ⚠️  Stooq {sembol}: CSV boş veya çok az veri")
            return None
        ...
        if len(df) < 2:
            print(f"  ⚠️  Stooq {sembol}: Filtreleme sonrası veri kalmadı")
            return None
        return df
    except Exception as e:
        print(f"  ⚠️  Stooq {sembol} hata: {e}")
        return None


def _gunluk_veri(sembol: str, gun: int = 60) -> Optional[pd.DataFrame]:
    """
    Günlük veri çek — önce yfinance, olmazsa Stooq.
    yfinance daha güvenilir, Stooq bot koruması olabilir.
    """
    # Yöntem 1: yfinance (öncelikli)
    try:
        df = _indir(sembol, interval="1d", period="3mo")
        if df is not None and len(df) >= gun:
            # Bugünün kısmi barını çıkar — timezone uyumsuzluğunu önle
            # yfinance index timezone-aware (America/New_York), normalize() timezone-naive
            # Çözüm: index'ten timezone'u kaldır
            df.index = df.index.tz_localize(None) if df.index.tz else df.index
            bugun = pd.Timestamp.now().normalize()
            df = df[df.index < bugun].tail(gun).copy()
            if len(df) >= 2:
                # Kolon isimlerini Stooq formatına uyumlu yap
                df = df.reset_index()
        ...
        print(f"  ⚠️  yfinance {sembol} hata: {e}")

    # Yöntem 2: Stooq (yedek)
    print(f"  📡 yfinance başarısız, Stooq deneniyor: {sembol}")
    return _stooq_gunluk(sembol, gun)
...
def _stooq_gunluk(sembol: str, gun: int = 60) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            return None
        if "Date" not in r.text:
            print(f"  ⚠️  Stooq {sembol}: CSV header 'Date' bulunamadı")
            return None
        df = pd.read_csv(StringIO(r.text))
        if df.empty or len(df) < 3:
            print(f"  ⚠️  Stooq {sembol}: CSV boş veya çok az veri")
...
def _dxy_cek() -> Optional[float]:
    """
    DXY (US Dollar Index) değerini çek.
    Öncelik sırası:
      1. yfinance DX-Y.NYB (doğrudan DXY spot)
      2. yfinance DX=F (DXY futures)
      3. Stooq DX.F (günlük veri)
      4. UUP ETF × 3.59 (yedek proxy)
    """
    # Yöntem 1: Doğrudan DXY (ICE spot)
    for sym in ["DX-Y.NYB", "DX=F"]:
        try:
            df = _indir(sym, interval="1d", period="5d")
            if df is not None and not df.empty:
                deger = float(df["Close"].iloc[-1])
                if deger > 50:  # DXY mantıklı aralık 70-130
                    return round(deger, 1)
        except:
            pass

    # Yöntem 2: Stooq DX.F (büyük harf önemli)
    try:
        df = _stooq_gunluk("DX.F", gun=10)
    ...
    # Yöntem 3: UUP ETF proxy (düzeltilmiş oran)
    try:
        uup = _indir("UUP", interval="1d", period="5d")
        if uup is not None and not uup.empty:
            uup_son = float(uup["Close"].iloc[-1])
            # UUP ≈ DXY / 3.59 (2026 güncel ampirik oran)
            # Örnek: UUP 27.70 ≈ DXY 99.47 → 99.47/27.70 ≈ 3.59
            return round(uup_son * 3.59, 1)
    except:
        pass
...
def _us10y_cek() -> Tuple[Optional[float], Optional[float], str]:
    """
    US 10Y Treasury Yield (ABD 10 yıllık tahvil faizi) çek.
    Öncelik:
      1. FRED API DGS10 (resmi Fed verisi, en güvenilir)
      2. yfinance ^TNX (CBOE 10Y T-Note Yield Index)
      3. yfinance ZN=F (10Y T-Note futures fiyatından ters çevirme)

    Returns: (son_deger, degisim_10g, kaynak)
    """
    # Yöntem 1: FRED API (ücretsiz, API key gerekli)
    fred_key = os.getenv("FRED_API_KEY", "")
    if fred_key:
        try:
            url = f"https://api.stlouisfed.org/fred/series/observations?series_id=DGS10&api_key={fred_key}&sort_order=desc&limit=15&file_type=json"
            r = requests.get(url, timeout=10)
            if r.status_code == 200:
                data = r.json()
                obs = data.get("observations", [])
                # FRED bazen '.' ile missing value gösterir
                valid = [float(o["value"]) for o in obs if o.get("value") and o["value"] != "."]
                if len(valid) >= 2:
                    son = valid[0]
                    on_gun = valid[min(10, len(valid)-1)]
                    return round(son, 2), round(son - on_gun, 2), f"FRED DGS10"
        except:
            pass

    # Yöntem 2: yfinance ^TNX
    try:
        df = _indir("^TNX", interval="1d", period="1mo")
        if df is not None and len(df) >= 10:
            # ^TNX bazen 10x ölçekli gelir (örn: 46.49 yerine 4.649)
            # Düzeltme: eğer > 20 ise 10'a böl
            son_raw = float(df["Close"].iloc[-1])
            son = son_raw / 10 if son_raw > 20 else son_raw
            on_gun_raw = float(df["Close"].iloc[-10])
            on_gun = on_gun_raw / 10 if on_gun_raw > 20 else on_gun_raw
            return round(son, 2), round(son - on_gun, 2), "yfinance ^TNX"
    except:
        pass

    # Yöntem 3: yfinance ^TNX (CBOE 10Y Yield Index)
    try:
        df = _indir("^TNX", interval="1d", period="1mo")
        if df is not None and len(df) >= 10:
            son_raw = float(df["Close"].iloc[-1])
    ...
    # Yöntem 4: Stooq 10YUSY.B — günlük CSV (yedek)
    try:
        df = _stooq_gunluk("10YUSY.B", gun=20)
        if df is not None and len(df) >= 10:
            son = float(df["Close"].iloc[-1])
            on_gun = float(df["Close"].iloc[-10])
            if 1 < son < 10:
                return round(son, 2), round(son - on_gun, 2), "Stooq 10YUSY.B"
    except:
        pass

    # Yöntem 4: 10Y T-Note futures (ZN=F) — fiyatı yield'e çevirme yaklaşık
    try:
        df = _indir("ZN=F", interval="1d", period="5d")
        if df is not None and not df.empty:
            fiyat = float(df["Close"].iloc[-1])
            # T-Note futures fiyatı ≈ 100 - yield*2 (basitleştirilmiş)
            tahmini_yield = (100 - fiyat / 2)
            if 1 < tahmini_yield < 10:
                return round(tahmini_yield, 2), None, "ZN=F proxy"
    except:
        pass
...
def s5_makro_dolar(df_gunluk: pd.DataFrame) -> Tuple[bool, str]:
    ...
    # DXY — SADECE Stooq DX.F (günlük CSV, temiz veri)
    dxy = None
    try:
        dxy = _gunluk_veri("DX-Y.NYB", gun=60)
    except:
        pass
    ...
    if dxy is not None and len(dxy) >= 10:
        try:
            ...
            dxy_10g    = float(dxy["Close"].iloc[-10]) if len(dxy) >= 10 else float(dxy["Close"].iloc[0])
    ...
    # 10Y Tahvil faizi — FRED DGS10 öncelikli, ^TNX yedek
    faiz_son, faiz_degisim, faiz_kaynak = _us10y_cek()
    if faiz_son is not None:
        degisim_str = f"{faiz_degisim:+.2f}" if faiz_degisim is not None else "?"
        if faiz_degisim is not None and faiz_degisim < -0.05:
            puan += 1
            notlar.append(f"ABD 10Y faiz:%{faiz_son:.2f} ({degisim_str}↓)✓ [{faiz_kaynak}]")
        elif faiz_degisim is not None and faiz_degisim > 0.05:
            notlar.append(f"ABD 10Y faiz:%{faiz_son:.2f} ({degisim_str}↑)✗ [{faiz_kaynak}]")
        else:
            notlar.append(f"ABD 10Y faiz:%{faiz_son:.2f} ({degisim_str})→ [{faiz_kaynak}]")
    else:
        notlar.append("10Y Faiz:?")
...
def enstruman_analiz(isim: str, cfg: dict) -> dict:
    ...
    df_stooq = _gunluk_veri(cfg["stooq"], gun=60)
...
def makro_yorum_uret(sonuclar: list) -> str:
    ...
    try:
        # DXY — SADECE Stooq DX.F
        dxy = _gunluk_veri("DX-Y.NYB", gun=5)
...
def _s5_aciklama(detay: str, isim: str = "ALTIN") -> str:
    ...
    faiz_yorum = ""
    if faiz_b:
        faiz_upper = faiz_b.upper()
        if ("↑" in faiz_b or "YÜKSEL" in faiz_upper or "+" in faiz_b) and "✗" in faiz_b:
            faiz_skor = -1
            faiz_yorum = "📈 Faiz yükseliyor → tahvil/banka cazip, yatırımcı altından çıkıyor"
        elif ("↓" in faiz_b or "DÜŞ" in faiz_upper or "-" in faiz_b) and "✓" in faiz_b:
            faiz_skor = 1
            faiz_yorum = f"📉 Faiz düşüyor → bankada tutmak yerine {hedef} daha mantıklı"
        elif "→" in faiz_b:
            faiz_yorum = f"📊 Faiz yatay → {hedef} için nötr etki"
...
def ai_tahmin_uret(sonuclar: list, takvim: list, haberler: list) -> str:
    ...
    if CEREBRAS_AKTIF and cerebras_key:
        try:
            ...
            model_adi = "gpt-oss-120b"  # llama-3.3-70b deprecated (2026-02-16)
        except:
            client = None
    if client is None and GROQ_AKTIF and GROQ_API_KEY:
        try:
            client    = Groq(api_key=GROQ_API_KEY)
            model_adi = "llama-3.3-70b-versatile"  # Groq'da hâlâ aktif
        except:
            pass
    if client is None:
        return ""
...
Yukarıdaki verilere dayanarak her iki metal için derinlemesine analiz yap."""
    ...
    try:
        ...
        if "rate_limit" in hata.lower() or "429" in hata:
            ...
            diger_client = None
            diger_model  = None
            if model_adi == "gpt-oss-120b" and GROQ_AKTIF and GROQ_API_KEY:
                try:
        ...
        print(f"  AI analiz hatası ({model_adi}): {hata[:150]}")
        return ""
...
def alarm_calistir(ai_aktif: bool = False):
    ...
    if ai_aktif:
        ...
        print("  🤖 AI tahmin: atlandı (--ai flag'i ekleyin, veya GROQ_API_KEY/CEREBRAS_API_KEY kontrol edin)")
