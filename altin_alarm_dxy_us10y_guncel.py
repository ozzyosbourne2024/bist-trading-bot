...
def _us10y_cek() -> Tuple[Optional[float], Optional[float], str]:
    ...
    # Yöntem 3: Stooq 10YUSY.B — günlük CSV (çok sağlam yedek)
    try:
...
def s5_makro_dolar(df_gunluk: pd.DataFrame) -> Tuple[bool, str]:
    ...
    # DXY — önce yfinance doğrudan, sonra stooq, en son UUP proxy
    dxy = None

    # 1. yfinance DX-Y.NYB veya DX=F
    for sym in ["DX-Y.NYB", "DX=F"]:
        try:
            dxy = _indir(sym, interval="1d", period="3mo")
    ...
    # 2. Stooq DX.F (büyük harf)
    if dxy is None:
        try:
            dxy = _stooq_gunluk("DX.F", gun=60)
        except:
            pass

    # 3. UUP ETF proxy (düzeltilmiş oran: 3.59)
    if dxy is None:
        try:
            uup = _indir("UUP", interval="1d", period="3mo")
            if uup is not None and len(uup) >= 20:
                dxy = uup.copy()
                dxy["Close"] = dxy["Close"] * 3.59
                dxy["High"]  = dxy["High"]  * 3.59
                dxy["Low"]   = dxy["Low"]   * 3.59
        except:
            pass
...
def makro_yorum_uret(sonuclar: list) -> str:
    ...
    try:
        # DXY — doğrudan yfinance DX-Y.NYB veya DX=F
        dxy = None
        for sym in ["DX-Y.NYB", "DX=F"]:
            dxy = _indir(sym, interval="1d", period="5d")
            if dxy is not None and len(dxy) >= 2:
                break
            dxy = None
        # Olmadı Stooq DX.F
        if dxy is None:
            dxy = _stooq_gunluk("DX.F", gun=5)
        if dxy is not None and len(dxy) >= 2:
